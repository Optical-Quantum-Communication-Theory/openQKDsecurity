...
function parameters=setParameters()
    ...
    parameters.names = ["pz","thetaA","thetaB","eta","pd","mu1","mu2","mu3","f"]; %MDIBB84
...
%     parameters.scan.thetaA = 0:pi/64:pi/4; %misalignment angle between Alice-Charlie
    parameters.scan.eta = flip(10.^(0:0.2:0)); %channel transmittance between Alice-Charlie (half of total distance)
...
%     parameters.fixed.eta = 1; %channel transmittance between Alice-Charlie (half of total distance)
    parameters.fixed.thetaA = 0.2; %misalignment angle between Alice-Charlie
    parameters.fixed.thetaB = 0; %misalignment angle between Bob-Charlie
    parameters.fixed.pd = 1e-6; %dark count probability
    parameters.fixed.f = 1; %error-correction efficiency
    
    %optimal parameters:
    parameters.fixed.mu1 = 0.25; %first intensity (signal)
...
%     parameters.optimize.mu1 = [0.02,0.25,0.6]; %first intensity (signal)
